...
class WriteItNode {
  ...
  init() {
    ...
    if (this.node.hasAttribute(WriteItJS.WRITEIT_REPLACE_NEXT)) {
      this.originalTexts = this.node
        ...
        .map((s) => s.split("").reverse().join("").replace("\\,", ","))
        .reverse();
      // remove redudant spaces
    ...
    while (
      ...
      if (
        waitTempIndex > -1 &&
        (waitTempIndex < writeTempIndex || writeTempIndex == -1)
      ) {
        // set wait.
        ...
        this.text = this.text.replace(
          this.text.substring(i, this.text.indexOf("}", i) + 1),
          ""
        );
        temp += 1;
      } else if (
        writeTempIndex > -1 &&
        (writeTempIndex < waitTempIndex || waitTempIndex == -1)
      ) {
        // set write.
        i = writeTempIndex;
        ...
        if (endIndex != -1) {
          ...
          this.text = this.text.replace(
            this.text.substring(i, endIndex + 1),
            this.text.substring(i + 2, endIndex)
          );
          temp += endIndex;
        }
      }
        ...
        while (
          ...
          if (
            ...
            tempText = tempText.replace(
              tempText.substring(i, tempText.indexOf("}", i) + 1),
              ""
            );
            temp += 2;
          } else if (
            ...
            if (
              this.node.hasAttribute(WriteItJS.WRITEIT_WRITE_ALL_IN_REVERSE)
            ) {
              this.writeAllTextAtOnceIndex[iterator][endIndex] = i + 1;
            }
            tempText = tempText.replace(
              tempText.substring(i, endIndex + 1),
              tempText.substring(i + 2, endIndex)
            );
            temp += endIndex + 1;
          }
        }
  ...
  setSpeed() {
    ...
    if (this.node.hasAttribute(WriteItJS.WRITEIT_SPEED_FIXED)) {
      this.speed =
        1000 /
        parseFloat(this.node.getAttribute(WriteItJS.WRITEIT_SPEED_FIXED));
  ...
  animate() {
    ...
    if (
      this.waitIndex[this.textsIndex < 0 ? "default" : this.textsIndex][
        this.index
      ] != undefined &&
      !this.wait
    ) {
    ...
    if (
      this.writeAllTextAtOnceIndex[
        this.textsIndex < 0 ? "default" : this.textsIndex
    ...
    if (this.reverse) {
      ...
      if (this.node.hasAttribute(WriteItJS.WRITEIT_REPLACE_NEXT)) {
        ...
        if (
          ...
          !(
            this.node.hasAttribute(WriteItJS.WRITEIT_LOOP) ||
            this.node.hasAttribute(WriteItJS.WRITEIT_LOOP_REVERSE)
          )
        ) {
          this.animationEnd();
          return;
  ...
  lastWriteItBlinkAnimation() {
    this.interval = this.setInterval(() => {
      if (
        this.node.innerHTML.replace(/'/g, '"') !=
        (this.text + this.writeitChar).replace(/'/g, '"')
      ) {
        this.node.innerHTML = this.text + this.writeitChar;
      } else {
        this.node.innerHTML =
          this.text + "\u00A0".repeat(this.writeitChar.length);
      }
    }, 500);
  }
